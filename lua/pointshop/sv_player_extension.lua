...
function Player:PS_BuyItem(item_id)
	...
	self:PS_GiveItem(item_id) -- MTZ
	self:PS_EquipItem(item_id) -- MTZ
end
...
function Player:PS_SellItem(item_id)
	...
	ITEM:OnHolster(self)  -- MTZ?
	ITEM:OnSell(self)  -- MTZ?
...
-- MTZ added
function Player:PS_HolsterFromCategory(cat_name)
	for item_id, item in pairs(self.PS_Items) do
		local ITEM = PS.Items[item_id]
		if ITEM.Category == cat_name and item.Equipped then
			self:PS_HolsterItem(item_id)
		end
	end
end
